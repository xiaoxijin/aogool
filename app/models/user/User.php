...
//            'is_promoter'=>$spread_uid || $storeBrokerageStatu != 1 ? 1: 0,
     ...
     * @throws \think\exception\DbException
     */
    public static function backMyselfOrderBrokerage($orderInfo)
    {
        //TODO 如果时营销产品不返佣金
        ...
        if($storeBrokerageStatus == 1){
            if(!User::be(['uid'=>$userInfo['uid'],'is_promoter'=>1])) return false;
        }
        //自己分佣比例
        $storeBrokerageRatio = SystemConfigService::get('store_brokerage_myself');
        ...
        //TODO 获取上级推广员信息
        $spreadUserInfo = User::getUserInfo($userInfo['uid']);
        //TODO 上级推广员返佣之后的金额
        ...
        self::beginTrans();
        //TODO 添加推广记录
        $res1 = UserBill::income('获得推广佣金',$userInfo['uid'],'now_money','brokerage',$brokeragePrice,$orderInfo['id'],$balance,$mark);
        //TODO 添加用户余额
        $res2 = self::bcInc($userInfo['uid'],'brokerage_price',$brokeragePrice,'uid');
    ...
    public static function backOrderBrokerage($orderInfo, bool $open = true)
    {
        ...
        self::backMyselfOrderBrokerage($orderInfo);
        ...
        $brokeragePrice = StoreProduct::getProductBrokerage($cartId ,1);
        //TODO 返佣金额小于等于0 直接返回不返佣金
        if ($brokeragePrice <= 0) return true;
        //TODO 获取上级推广员信息
...
//        if($res) return self::backOrderBrokerageTwo($orderInfo);
    ...
    public static function backOrderBrokerageTwo($orderInfo, bool $open = true)
    {
        ...
        $cartId = is_string($orderInfo['cart_id']) ? json_decode($orderInfo['cart_id'], true) : $orderInfo['cart_id'];
        $brokeragePrice = StoreProduct::getProductBrokerage($cartId, 2);
        //TODO 返佣金额小于等于0 直接返回不返佣金
        if ($brokeragePrice <= 0) return true;
        //TODO 获取上上级推广员信息
    ...
    public static function backOrderBrokerageThree($orderInfo, bool $open = true)
    {
        ...
        //TODO 上推广人不存在 或者 上推广人没有上级  或者 当用用户上上级时自己  直接返回
        if (!$userInfoThree || !$userInfoThree['spread_uid'] || $userInfoThree['spread_uid'] == $orderInfo['uid']) return true;
        //TODO 获取后台分销类型  1 指定分销 2 人人分销
        if (!User::be(['uid' => $userInfoThree['spread_uid'], 'is_promoter' => 1])) return true;
        $cartId = is_string($orderInfo['cart_id']) ? json_decode($orderInfo['cart_id'], true) : $orderInfo['cart_id'];
        $brokeragePrice = StoreProduct::getProductBrokerage($cartId, 3);
        //TODO 返佣金额小于等于0 直接返回不返佣金
        if ($brokeragePrice <= 0) return true;
        //TODO 获取上上级推广员信息
        $spreadUserInfoTwo = User::getUserInfo($userInfoThree['spread_uid']);
        //TODO 获取上上级推广员返佣之后余额
        $balance = bcadd($spreadUserInfoTwo['brokerage_price'], $brokeragePrice, 2);
        $mark = '三级推广人' . $userInfo['nickname'] . '成功消费' . floatval($orderInfo['pay_price']) . '元,奖励推广佣金' . floatval($brokeragePrice);
        $open && self::beginTrans();
        //TODO 添加返佣记录
        $res1 = UserBill::income('获得推广佣金', $userInfoThree['spread_uid'], 'now_money', 'brokerage', $brokeragePrice, $orderInfo['id'], $balance, $mark);
        //TODO 添加用户余额
        $res2 = self::bcInc($userInfoThree['spread_uid'], 'brokerage_price', $brokeragePrice, 'uid');
        $res = $res1 && $res2;
        $open && self::checkTrans($res);
        return $res;
    }
